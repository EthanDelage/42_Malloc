...
static void test_chunk_coalesce_adjacent_chunks(void) {
    size_t size1 = 64;
    size_t size2 = 128;
    size_t total = sizeof(chunk_header_t) + size1 + sizeof(chunk_header_t) + size2;

    chunk_header_t *mem = malloc(total);

    chunk_header_t *chunk = mem;
    chunk_header_t *next = (chunk_header_t *)((char *)chunk + sizeof(chunk_header_t) + size1);

    *chunk = (chunk_header_t){.size = size1, .in_use = 0, .next = next, .previous = NULL};
    *next = (chunk_header_t){.size = size2, .in_use = 0, .next = NULL, .previous = chunk};
...
static void test_chunk_coalesce_updates_links(void) {
    ...
    chunk_header_t *chunk = mem;
    chunk_header_t *next = (chunk_header_t *)((char *)chunk + sizeof(chunk_header_t) + size1);
    chunk_header_t *after = (chunk_header_t *)((char *)next + sizeof(chunk_header_t) + size2);

    *chunk = (chunk_header_t){.size = size1, .in_use = 0, .next = next, .previous = NULL};
    *next = (chunk_header_t){.size = size2, .in_use = 0, .next = after, .previous = chunk};
    *after = (chunk_header_t){.size = size3, .in_use = 0, .next = NULL, .previous = next};
